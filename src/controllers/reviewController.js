...
const updateReview = async function (req, res) {
    try {
        ...
        const review1 = await reviewModel.findOne({ bookId: bookId, _id: reviewId, isDeleted: false });
        ...
        await reviewModel.findByIdAndUpdate({ _id: reviewId, }, { reviewedBy: reviewedBy, rating: rating, review: review, reviewedAt: new Date() }, { new: true }).select({ "_id": 1, "bookId": 1, "reviewedBy": 1, "reviewedAt": 1, "rating": 1, "review": 1 })
...
const deleteReview = async function (req, res) {
    try {
        ...
        await bookModel.findOneAndUpdate({ _id: bookId, }, { reviews: book.reviews - 1 }, { new: true })

        res.status(200).send({ status: true, message: `${reviewId} this review is deleted successfully` })
